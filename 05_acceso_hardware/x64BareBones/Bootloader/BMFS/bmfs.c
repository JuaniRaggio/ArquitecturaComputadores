...
int main(int argc, char *argv[]) {
        ...
        } else // Opened ok, is it a valid BMFS disk?
        {
                ...
                fseek(disk, 1024,
                      SEEK_SET); // Seek 1KiB in for disk information
                fread(DiskInfo, 512, 1,
                      disk); // Read 512 bytes to the DiskInfo buffer
                fseek(disk, 4096, SEEK_SET); // Seek 4KiB in for directory
                fread(Directory, 4096, 1,
                      disk); // Read 4096 bytes to the Directory buffer
                rewind(disk);

                if (strcasecmp(DiskInfo, fs_tag) !=
                    0) // Is it a BMFS formatted disk?
                {
                        if (strcasecmp(s_format, command) == 0) {
                                format();
                        } else {
                                printf("Error: Not a valid BMFS drive (Disk is "
                                       "not BMFS formatted).\n");
                        }
                        fclose(disk);
                        return 0;
        ...
        if (strcasecmp(s_list, command) == 0) {
                ...
                if (filename == NULL) {
                        ...
                        if (argc > 4) {
                                ...
                                fgets(tempstring, 32,
                                      stdin); // Get up to 32 chars from the
                                              // keyboard
        ...
        for (tint = 0; tint < 64; tint++) // Max 64 entries
        {
                ...
                } else // Valid entry
                {
                        printf("%-32s %20lld %20lld\n", entry.FileName,
                               entry.FileSize, (entry.ReservedBlocks * 2));
                }
        }
}
...
int initialize(char *diskname, char *size, char *mbr, char *boot,
        ...
        for (i = 0; size[i] != '\0' && ret == 0; ++i) {
                ...
                if (isdigit(ch)) {
                        unsigned int n = ch - '0';
                        if (diskSize * 10 >
                            diskSize) // Make sure we don't overflow
                        {
                                diskSize *= 10;
                                diskSize += n;
                ...
                {
                        printf(
                            "Error: A numeric disk size must be specified\n");
                        ret = 1;
                } else {
                        switch (toupper(ch)) {
                        case 'K':
                        ...
                        default:
                                printf(
                                    "Error: Invalid disk size string: '%s'\n",
                                    size);
                                ret = 1;
                                break;
                        }

                        // If this character is a valid unit indicator, but is
                        // not at the end of the string, then the string is
                        // invalid.
                        if (ret == 0 && size[i + 1] != '\0') {
                                printf(
                                    "Error: Invalid disk size string: '%s'\n",
        ...
        if (ret == 0) {
                if (diskSize < minimumDiskSize) {
                        printf("Error: Disk size must be at least %llu bytes "
                               "(%lluMiB)\n",
                               minimumDiskSize,
        ...
        // Open the disk image file for writing.  This will truncate the disk
        // file if it already exists, so we should do this only after we're
        // ready to actually write to the file.
        if (ret == 0) {
        ...
        if (ret == 0) {
                ...
                while (writeSize < diskSize) {
                        ...
                        percent *= 100;
                        printf(
                            "Formatting disk: %llu of %llu bytes (%.0f%%)...\r",
                ...
                if (ret == 0) {
                        printf(
                            "Formatting disk: %llu of %llu bytes (100%%)%9s\n",
                            writeSize, diskSize, "");
                }
        }
...
void create(char *filename, unsigned long long maxsize) {
        ...
        if (findfile(filename, &tempentry, &slot) == 0) {
                ...
                int num_used_entries = 0;  // how many entries of Directory are
                                           // either used or deleted
                ...
                for (tint = 0; tint < 64; tint++) {
                        ...
                        if (pEntry->FileName[0] == 0x00) // end of directory
                        {
                                ...
                                if (first_free_entry == -1)
                                        first_free_entry =
                                            tint; // there were no unused
                                                  // entires before, will use
                                                  // this one
                                break;
                ...
                if (first_free_entry == -1) {
                        printf(
                            "Cannot create file: no free directory entries.\n");
                        return;
                }
                ...
                for (tint = 0; tint < num_used_entries + 1; tint++) {
                        // on each iteration of this loop we'll see if a new
                        // file can fit between the end of the previous file
                        // (initially == 1) and the beginning of the current
                        // file (or the last data block if there are no more
                        // files).
                        ...
                        if (tint == num_used_entries ||
                            pEntry->FileName[0] == 0x01)
                                this_file_start =
                                    num_blocks - 1; // index of the last block
                        else
                        ...
                        if (tint < num_used_entries)
                                prev_file_end = pEntry->StartingBlock +
                                                pEntry->ReservedBlocks;
                }
                ...
                pEntry =
                    (struct BMFSEntry *)(Directory + first_free_entry * 64);
                pEntry->StartingBlock  = new_file_start;
                pEntry->ReservedBlocks = blocks_requested;
                pEntry->FileSize       = 0;
                strcpy(pEntry->FileName, filename);

                if (first_free_entry == num_used_entries &&
                    num_used_entries + 1 < 64) {
                        // here we used the record that was marked with 0x00,
                        // so make sure to mark the next record with 0x00 if it
                        // exists
                        pEntry =
                            (struct BMFSEntry *)(Directory +
                                                 (num_used_entries + 1) * 64);
                ...
                fseek(disk, 4096, SEEK_SET); // Seek 4KiB in for directory
                fwrite(Directory, 4096, 1,
                       disk); // Write 4096 bytes for the Directory

                //		printf("Complete: file %s starts at block %lld,
                // directory entry
...
void write(char *filename) {
        ...
        if (0 == findfile(filename, &tempentry, &slot)) {
                printf("Error: File not found in BMFS. A file entry must first "
                       "be created.\n");
        } else {
                ...
                if ((tfile = fopen(filename, "rb")) == NULL) {
                        ...
                        rewind(tfile);
                        if ((tempentry.ReservedBlocks * 2097152) <
                            tempfilesize) {
                                printf("Not enough reserved space in BMFS.\n");
                        } else {
                                fseek(disk, tempentry.StartingBlock * 2097152,
                                      SEEK_SET); // Skip to the starting block
                                                 // in the disk
                                for (tint = 0; tint < tempfilesize; tint++) {
                                        putc(getc(tfile),
                                             disk); // This is really terrible.
                                        // TODO: Rework with fread and fwrite
                                        // (ideally with a 2MiB buffer)
                                }
                                // Update directory
                                memcpy(Directory + (slot * 64) + 48,
                                       &tempfilesize, 8);
                                fseek(disk, 4096,
                                      SEEK_SET); // Seek 4KiB in for directory
                                fwrite(Directory, 4096, 1,
                                       disk); // Write new directory to disk
                                printf("Complete\n");
                        }
                        fclose(tfile);
                }
