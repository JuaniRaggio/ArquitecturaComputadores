...
num2str:
    ...
    mov   rax, [rbp + 8]  ;; Me guardo una copia del numero
    mov   rdi, [rbp + 12]
    mov   rbx, 10         ;; Divisor
    mov   rcx, 0          ;; Contador
...
get_current_time:
    ...
    out   70h, al
    in    ax, 71h
    push  rcx           ; Paso el inicio del buffer para que escriba ahi la h
    ...
    out   70h, al
    in    ax, 71h
    push  [rcx + 3]     ; Le paso la posicion en la que tiene que escribir
    push  ax
    call  num2str
